...
int main(int argc, char** argv)
{
    ...
    // std::vector<place_t> waypoints;  // TODO use a vector for waypoint navigation
    ...
    while(nh.ok()) {
        ...
        if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED) {
            string stop_arrive = base_sound_path + phrases[1];
            string delivered = base_sound_path + phrases[2];

            sc.playWave(stop_arrive);
            ROS_INFO("Playing stop_arrive");
            sleepok(6, nh);
            sc.stopWave(stop_arrive);
            ROS_INFO("Press enter to continue to next goal.");
            std::cin;
            sc.playWave(delivered);
        }
        else {
